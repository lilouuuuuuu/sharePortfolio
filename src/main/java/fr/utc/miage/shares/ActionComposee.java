...
public class ActionComposee extends Action{
    private HashMap<Action, Float> actions;
    ...
    public float valeur(Jour j) {
        ...
        for (Action action : actions.keySet()) {
            float part = actions.get(action);
            valeurTotale += part * action.valeur(j);
        }
        return valeurTotale;
    }

    public HashMap<Action, Float> getActions() {
        return actions;
    }

    public void setActions(HashMap<Action, Float> actions) {
        if (actions == null) {
            throw new IllegalArgumentException("La liste d'actions ne peut pas être null.");
        }
        this.actions = actions;
